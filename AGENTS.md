...
- `layouts/partials/head/css.html` and `layouts/partials/head/js.html` override the theme's partials to append `assets/css/custom.css` and `assets/js/custom.js` (the JS drives the CV skill-bar fill animation). When bumping the Mana submodule, diff both files against their `themes/mana/layouts/partials/head/` counterparts and re-sync.
