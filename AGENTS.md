...
- Comments: giscus (GitHub Discussions on this repo, category *Announcements*) is configured in `[params.giscus]`, exposed as `window.siteGiscus` by the js partial, and mounted by `custom.js` after `.post-content-main` on any page lacking the `.page-meta-hidden` marker (i.e. blog posts only). Its light/dark theme follows Mana's toggle via a MutationObserver. Removing the params block disables comments entirely.
